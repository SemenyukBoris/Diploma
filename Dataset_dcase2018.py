...
class DCASE2018_Task4_DevelopmentSet(AudioTaggingDataset):
    ...
    def __init__(self,
        ...
        kwargs['package_list'] = [
            {
                'content_type': 'meta',
                'filename': 'dataset/metadata/train/train.csv'
            },
            {
                'content_type': 'meta',
...
# dataset/metadata/train/train.csv
# ataset/audio/train/
...
#                or not os.path.isfile(evaluate_filename) or not self.meta_container.exists():
        if not os.path.isfile(train_filename_fold2) or not os.path.isfile(test_filename_fold2) \
                or not os.path.isfile(evaluate_filename) or not self.meta_container.exists():
        ...
        if not evaluation_setup_exists:
            ...
            audio_path = 'dataset/audio/train'
            for item in MetaDataContainer().load(os.path.join(self.local_path, 'dataset/metadata/train/train.csv'),
                                                 fields=["filename", "tags"], csv_header=True):
                if item.filename not in non_existing_videos:
                    if not item.filename.endswith(self.default_audio_extension):
                        item.filename = os.path.join(
            ...
            train_meta_weak_fold2.save(filename=train_filename_fold2, csv_header=True, file_format="CSV")
...
#            meta_data += MetaDataContainer().load(test_filename_fold1, csv_header=True, file_format="CSV")
            meta_data += MetaDataContainer().load(train_filename_fold2, csv_header=True, file_format="CSV")
